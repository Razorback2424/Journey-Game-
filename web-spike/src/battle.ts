...
export interface EnemyResolution { movedUnitId?: string; attackerId?: string; targetId?: string; }
...
function runEnemyTurn(state: BattleState): EnemyResolution { const enemy = activeUnit(state); if (!enemy || enemy.team !== 'enemy' || state.winner) return {}; const resolution: EnemyResolution = {}; const players = living(state, 'player'); const target = players.sort((a, b) => a.hp / a.maxHp - b.hp / b.maxHp || distance(enemy, a) - distance(enemy, b) || a.id.localeCompare(b.id))[0]; if (!target) return resolution; if (!validAttackTargets(state, enemy).length) { const moves = validMoveTiles(state, enemy); const next = moves.sort((a, b) => distance(a, target) - distance(b, target) || movementCost(state, enemy, a) - movementCost(state, enemy, b))[0]; if (next) { enemy.x = next.x; enemy.y = next.y; enemy.moved = true; resolution.movedUnitId = enemy.id; pushEvent(state, { message: `${enemy.name} closes in.`, kind: 'info' }); } } const victim = validAttackTargets(state, enemy).sort((a, b) => a.hp / a.maxHp - b.hp / b.maxHp || a.id.localeCompare(b.id))[0]; if (victim) { enemy.actionUsed = true; resolution.attackerId = enemy.id; resolution.targetId = victim.id; applyDamage(state, enemy, victim); } return resolution; }
export function endPlayerTurn(state: BattleState) { const current = activeUnit(state); if (state.winner || !current || current.team !== 'player') return false; pushEvent(state, { message: `${current.name} ends their turn.`, kind: 'info' }); advanceTurn(state); return true; }
export function resolveActiveEnemyTurn(state: BattleState) { if (state.winner || activeUnit(state)?.team !== 'enemy') return {}; const resolution = runEnemyTurn(state); if (!state.winner) advanceTurn(state); return resolution; }
